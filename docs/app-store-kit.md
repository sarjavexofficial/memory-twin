# App Store 提出セット(2026-07-12作成 / 2026-07-17 課金あり版に再改訂)

Apple Developer Program登録後、App Store Connectにコピペしていくための素材集。

> **初回リリース方針(2026-07-17 ゆず最終決定): v1.0から本物のApp内課金を提供する。**
> 無料 / Standard ¥980月・¥9,800年 / Pro ¥1,980月・¥19,800年（自動更新サブスクリプション）。
> billing.ts は RevenueCat 実装に接続済み・デモ購入UIは撤去済み（購入=Apple標準シート、
> 解約=Appleのサブスク管理画面）。提出前に必須:
> ①ASCの有料App契約(銀行・税務) ②サブスク4商品の登録 ③RevenueCat設定とiOSキー
> ④キーをEAS環境変数に登録して**ビルド8**を作成（課金はネイティブ機能のためEAS Updateでは届かない）
> ⑤Sandboxで購入・復元・アップグレードのテスト ⑥IAP商品をアプリと同時に審査提出
...
| カテゴリ(セカンダリ) | 仕事効率化 |
| 価格 | 無料(App内課金あり) |
| App内課金 | Standard ¥980/月・¥9,800/年、Pro ¥1,980/月・¥19,800/年(自動更新サブスクリプション・グループ内ランク: Pro=1, Standard=2) |
...
■ 料金
基本機能は無料。もっと使いたくなったら:
・Standard(¥980/月・¥9,800/年): AI質問 月500回、履歴取り込み無制限、週次・月次のAI振り返り
・Pro(¥1,980/月・¥19,800/年): AI質問 月1,500回、Today Recall最大10件、過去比較・自動学習
年払いなら2か月分お得です。
...
■ Pricing
Core features are free. Standard (¥980/mo, ¥9,800/yr) and Pro (¥1,980/mo, ¥19,800/yr)
unlock more AI questions, unlimited history imports, AI-written reviews, and more.
Cancel anytime from iPhone Settings.
...
7. In-app purchases: two auto-renewable subscriptions (Standard / Pro, monthly
   and yearly) via Apple IAP (RevenueCat SDK). "Restore Purchases" is available
   on the Plans screen (Settings > Plans > Payment section). Cancellation is
   handled by Apple's subscription management, linked from the same screen.
   Terms of Use (Apple standard EULA) and Privacy Policy links are in
   Settings > About.
```
